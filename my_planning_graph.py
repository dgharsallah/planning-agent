...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        """
#         return True
        for A_effect in self.children[actionA]:
            for B_effect in self.children[actionB]:
    ...
    def _interference(self, actionA, actionB):
        ...
        for effect in self.children[actionA]:
            for precondition in self.parents[actionB]:
                if effect == ~precondition:
                    return True
        for effect in self.children[actionB]:
            for precondition in self.parents[actionA]:
                if effect == ~precondition:
                    return True
        return False
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        for A_precondition in self.parents[actionA]:
            for B_precondition in self.parents[actionB]:
                if self.parent_layer.is_mutex(A_precondition, B_precondition):
                    return True
        return False
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        for goal in self.goal:
            found = False
            for i in range(len(self.literal_layers)):
                for lit in self.literal_layers[i]:
                    if lit == goal:
                        sum = i + sum
                        found = True
                        break
    ...
    def h_setlevel(self):
        ...
        for i in range(len(self.literal_layers)):
            all_goals_met = True
            for goal in self.goal:
                if not goal in self.literal_layers[i]:
                    all_goals_met = False
